...
SRAM（Static Random Access Memory，静态随机存取存储器）是一类在断电前能持续保持数据的半导体寄存阵列，它以锁存器作为基本存储单元，不需要像 DRAM 那样周期性刷新，因此具备访问延迟低、功耗稳定的特点。SRAM 通常部署在芯片内部，作为离算力核心最近的存储层级，用于快速缓存逻辑单元产生或消费的数据。常见的应用场景包括：

- 处理器与加速器内的 L1/L2 Cache、TLB 等高速缓冲。
- 多媒体、AI 内核中的权重缓冲、片上图像块存储、激活值缓存。
- 通信协议栈中的包缓冲、DMA 环形队列、网络交换中继存储。
- FPGA/ASIC 中的寄存文件（register file）或可配置 LUT RAM。

在 assassyn 中学习 SRAM 的建模与调度，有助于在硬件设计阶段快速搭建上述存储子系统原型，并评估不同架构下的时序与资源取舍。
...
> 注意：assassyn 中的 SRAM 提供同步读写接口，读出的数据通过 RegArray 暴露(如果没有读操作，任然保持上一个读的结果)，需要在下游模块中组合使用。
...
> hint: 
> 
> 1. 目前 `SRAM` 的总大小是 `512bytes` , 一个元素的大小是 `256bits`，因此最多只能存储 `16` 个元素。请不要让资源文件超过这个大小。
